                ...
                if(!empty($_FILES['pdf']['tmp_name']))
                {
                    ...
                    if(!isset($fileError))
                    {
                        ...
                        if(move_uploaded_file($_FILES['pdf']['tmp_name'], $dossier.$pdfcpt))
                        {
                            ...
                            $upload = $bdd->prepare("UPDATE works SET title=:title, category=:category,pdf=:pdf,description=:description, year=:year WHERE id=:myid");
                            $upload->execute([
                                ":title" => $title,
                                ":category" => $category,
                                ":pdf"=>$pdfcpt,
                    ...
                    $upload = $bdd->prepare("UPDATE works SET title=:title, category=:category,description=:description, year=:year WHERE id=:myid");
                    $upload->execute([
                        ":title" => $title,
                        ":category" => $category,
                        ":description" => $description,
                        ...
                        if(move_uploaded_file($_FILES['image']['tmp_name'], $dossier.$fichiercpt))
                        {
                            ...
                            if(empty($_FILES['pdf']['tmp_name']))
                            {
                                ...
                                // faire un update des info
                                    $upload = $bdd->prepare("UPDATE works SET title=:title, category=:category, image=:image, description=:description, year=:year WHERE id=:myid");
                                    $upload->execute([
                                        ":title" => $title,
                                        ":category" => $category,
                                        ":image" => $fichiercpt,
                                ...
                                if(move_uploaded_file($_FILES['pdf']['tmp_name'], $dossier.$pdfcpt))
                                {
                                    ...
                                    $upload = $bdd->prepare("UPDATE works SET title=:title, category=:category, image=:image, pdf=:pdf, description=:description, year=:year WHERE id=:myid");
                                    $upload->execute([
                                        ":title" => $title,
                                        ":category" => $category,
                                        ":image" => $fichiercpt,
